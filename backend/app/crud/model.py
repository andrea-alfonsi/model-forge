...
def create_model(db: Session, model: ModelCreateRequest, background_tasks: BackgroundTasks):
    db_model = Model(name=model.name,description=model.description,task=model.task,derived_from_id=model.derived_from_id,kind=model.kind.value)
    ...
    db.commit()
    db.refresh(db_model)
    task = celery_app.send_task("train_model", kwargs={"job_id": db_training.id, 'model_id': db_model.id, 'dataset_id': model.training_dataset_id, 'configuration_json': model.trainingHyperparameters.dict()}, queue="cpu")
